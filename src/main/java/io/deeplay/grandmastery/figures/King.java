...
public class King extends Piece {
  ...
  private boolean canCastling(Board board, Move move) {
    ...
    if (FigureUtils.hasFigureOnHorizontalBetweenColPosition(
  ...
  public List<Move> getAllMoves(Board board, Position position) {
    ...
    moves.add(FigureUtils.getMoveByPositionAndDeltas(position, 0, 1));
    moves.add(FigureUtils.getMoveByPositionAndDeltas(position, 1, 1));
    moves.add(FigureUtils.getMoveByPositionAndDeltas(position, 1, 0));
    moves.add(FigureUtils.getMoveByPositionAndDeltas(position, 1, -1));
    moves.add(FigureUtils.getMoveByPositionAndDeltas(position, 0, -1));
    moves.add(FigureUtils.getMoveByPositionAndDeltas(position, -1, -1));
    moves.add(FigureUtils.getMoveByPositionAndDeltas(position, -1, 0));
    ...
    if (!this.isMoved) {
      if (this.color == Color.WHITE) {
        moves.add(LongAlgebraicNotationParser.getMoveFromString("e1c1"));
        moves.add(LongAlgebraicNotationParser.getMoveFromString("e1g1"));
      } else {
        moves.add(LongAlgebraicNotationParser.getMoveFromString("e8c8"));
        moves.add(LongAlgebraicNotationParser.getMoveFromString("e8g8"));
      }
    }
